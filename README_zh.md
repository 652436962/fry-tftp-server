...
RustTFTP 是一个使用 Rust 开发的高性能 TFTP 服务器，专为网络工程师、系统管理员以及需要快速可靠地分发固件和搭建 PXE 启动环境的用户设计。支持所有主流 TFTP RFC 规范，包括滑动窗口传输（RFC 7440），吞吐量最高可达 **500+ MB/s**。
...
从 [Releases](https://github.com/qulisun/rust-tftp/releases) 下载 `.dmg`，打开后将 **RustTFTP** 拖入应用程序。

> **注意:** 首次启动时，请右键点击应用并选择 **打开**（macOS Gatekeeper）。macOS Ventura+ 上端口 69 无需 sudo 权限。

### Windows

从 [Releases](https://github.com/qulisun/rust-tftp/releases) 下载 `rust-tftp-windows-x86_64.zip`，解压后双击 `rust-tftp.exe`。
...
从 [Releases](https://github.com/qulisun/rust-tftp/releases) 下载 `rust-tftp-linux-x86_64.AppImage`，然后运行：
...
> ```bash
> mv rust-tftp-linux-x86_64.AppImage ~/.local/bin/rust-tftp
> cp deploy/appimage/rust-tftp.desktop ~/.local/share/applications/
> ```
> `.desktop` 文件已包含在 AppImage 中，位于 `deploy/appimage/rust-tftp.desktop`。
...
**GUI 模式** — 直接双击 `rust-tftp.exe`，无需控制台窗口，无需安装。
