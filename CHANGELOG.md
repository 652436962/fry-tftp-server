...
### Added
- Simplified Chinese (简体中文) GUI support
- Linux AppImage packaging and distribution
- Project renamed from "Fry TFTP Server" to "RustTFTP"
- Linux default log path differentiated by runtime medium (AppImage vs deb)
- GTK/CJK font fallback for Chinese characters in GUI
- Linux system tray integration with internationalized menu and tooltip

### Fixed
- Linux GTK initialization crash on startup
- Linux window icon not displaying properly
- Log file permission errors causing panic (now graceful degradation)
- Window close now hides to tray instead of quitting
- Clippy warnings fixed (sort_by, match guard style)
